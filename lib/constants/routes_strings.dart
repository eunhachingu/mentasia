class RoutesList {
  static String splash = "/";
  static String homeScreen = "/home_screen";

  // Chat Screen
  static String chatScreen = "/chat_screen";

  // Log In Screen
  static String loginScreen = "/login_screen";
  static String signupScreen = "/sign_up_screen"; // Settings Screen
  static String settingsScreen = "/settings_screen";

  // Account
  static String personalInformationScreen = "/personal_acount_screen";
  static String notifications = "/notifications_screen";

  // Legal
  static String termsOfService = "/terms_of_service_screen";

  // Support

  static String aboutUsScreen = "/about_us_screen";
  static String helpFeedbackScreen = "/help_feedback_screen";
  static String mentasiaWorksScreen = "/mentasia_works_screen";
}
